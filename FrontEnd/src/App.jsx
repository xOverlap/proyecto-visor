...
function App() {
    ...
    const handleClick = async (retryCount = 0) => {
        if (archiveType !== "") {
            setLoading(true)
            setDownload(false)
            let downloadFile; // Definir la variable antes del bloque try
            const MAX_RETRIES = 15 // Número máximo de intentos
            const RETRY_DELAY = 400 // Tiempo de espera en milisegundos antes de volver a intentar
            ...
            try {
                ...
                if (retryCount < MAX_RETRIES) {
                    setTimeout(() => handleClick(retryCount + 1), RETRY_DELAY)
                } else {
                    console.error(error)
                }
            } finally {
